...
export function useVideoPickerLogic() {
  ...
  async function pickVideo() {
    try {
      ...
      // Check video format (only accept MP4)
      const uri = asset.uri;
      const fileExtension = uri?.toLowerCase().split(".").pop() || "";

      if (!["mp4", "mov", "m4v"].includes(fileExtension)) {
        Alert.alert(
          "Unsupported video format",
          `Please select an MP4 video file.\n\nThe selected file (${fileExtension.toUpperCase()}) is not supported.`
        );
        return;
      }
      ...
      console.log(
        `[Video Picker] Selected video: ${fileSizeMB.toFixed(2)} MB, ${
          asset.duration
        }ms`
      );
      ...
      if (fileSizeMB > MAX_SIZE_MB) {
        Alert.alert(
          "Video too large",
          `This video is ${fileSizeMB.toFixed(
            1
          )} MB. Please select a video smaller than ${MAX_SIZE_MB} MB.`
        );
        return;
      }
      ...
      if (uri) {
        ...
        if (fileSizeMB < 10) {
          ...
          console.log(
            `[Video Picker] Video is ${fileSizeMB.toFixed(
              2
            )} MB - no compression needed`
          );
          setVideoUri(uri);
          setFileName("Selected video");
        } else {
  ...
  async function compressAndSetVideo(uri: string, originalSizeMB: number) {
    try {
      ...
      console.log(
        `[Video Picker] Compressing video (${originalSizeMB.toFixed(2)} MB)...`
      );
      ...
      const compressedUri = await Video.compress(
        ...
        (progress) => {
          console.log(
            `[Video Picker] Compression progress: ${(progress * 100).toFixed(
              0
            )}%`
          );
        }
      ...
      try {
        ...
        if (compressedFileInfo.exists && "size" in compressedFileInfo) {
          ...
          console.log(
            `[Video Picker] Compressed file size: ${compressedSizeMB.toFixed(
              2
            )} MB (${originalSizeMB.toFixed(2)} MB → ${compressedSizeMB.toFixed(
              2
            )} MB)`
          );
        }
        ...
        console.warn(
          "[Video Picker] Could not check compressed file size:",
          sizeCheckError
        );
      }
